...
export function ListGroup({ children }: { children: ReactNode }) {
  return (
    <div className="mx-4 divide-y divide-hairline overflow-hidden rounded-lg border border-hairline bg-surface shadow-soft">
      {children}
    </div>
  )
}
...
const VARIANTS: Record<Variant, string> = {
  ...
  checkout:
    'w-full bg-forest text-white shadow-checkout py-[18px] text-[18px] !rounded-lg hover:scale-[1.02]',
...
export function AddButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="grid h-11 w-11 shrink-0 place-items-center rounded bg-primary text-white transition-transform duration-150 ease-bounce hover:scale-105 active:scale-95"
    >
...
export function ProductCard({
  ...
  return (
    <div className="rounded-card border border-hairline bg-surface p-3">
      <div
        className={`relative mb-3 grid h-[120px] place-items-center rounded-lg bg-gradient-to-br ${gradient}`}
      >
...
export function ProductRow({
  ...
  return (
    ...
    >
      <div
        className={`relative grid h-[88px] w-[88px] shrink-0 place-items-center rounded-lg border border-black/10 bg-gradient-to-br ${gradient}`}
      >
      ...
      <div className="flex shrink-0 items-center gap-2.5">
        <span className="font-display text-[16px] font-bold text-ink">{price}</span>
        <AddButton label={`Add ${name}`} onClick={onAdd} />
      </div>
    </div>
  )
...
export function RecipeCard({
  ...
  return (
    <div className="rounded-card border border-hairline bg-surface p-3">
      <div
        className={`mb-3 grid h-[110px] place-items-center rounded-lg bg-gradient-to-br ${gradient}`}
      >
